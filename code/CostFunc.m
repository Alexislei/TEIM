function [ fitness,est_alpha] = CostFunc( x,F_rgb,B_rgb,U_rgb,F_s,B_s,U_s,F_mindist,B_mindist)
...
%     end
    ...
    tmp=est_alpha(:,ones(3,1)); %replicate the est_alpha column vector to 3 columns(rgb)
    % Chromatic distortion
%     cost_c  = norm2(U_rgb-(est_alpha.*Fx_rgb+(1-est_alpha).*Bx_rgb));
    cost_c=norm2(U_rgb-(tmp.*Fx_rgb+(1-tmp).*Bx_rgb));
    % Spatial cost
    cost_sF = norm2(Fx_s-U_s)./F_mindist;
    cost_sB = norm2(Bx_s-U_s)./B_mindist;
    cost_cd = norm2(Fx_Bx_rgb);
%     fitness = [cost_c;cost_sF;cost_sB];%
    fitness=1*cost_c+cost_sF+cost_sB;
%     fitness = sum(1*cost_c+cost_sF+cost_sB);
end
